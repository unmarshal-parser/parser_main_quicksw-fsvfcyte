...
func MethodIndexerCallback(orm *gorm.DB, setFeeToMethodTracker SetFeeToMethodTracker, setFeeToSetterMethodTracker SetFeeToSetterMethodTracker, createPairMethodTracker CreatePairMethodTracker, from, to int, contractAddress string, unmarshalSDKWrapper *UnmarshalSDKWrapper) error {
	errs, _ := errgroup.WithContext(context.Background())
		...
		for true {
			...
			for i := 0; i < parallelCalls; i++ {
				...
				go func(routineFrom, routineTo int) {
					...
					err = MethodIndexerCallback(orm, setFeeToMethodTracker, setFeeToSetterMethodTracker, createPairMethodTracker, routineFrom, routineTo, config.ContractAddress, unmarshalSDKWrapper)
					if err != nil {
